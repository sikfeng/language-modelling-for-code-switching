...
def load_data(sents_f, evaluate):
  ...
  if evaluate:
      ...
      for test_set in tqdm(sents_f, desc="Loading eval dataset"):
        ...
        for sent in [test_set["orig"]] + test_set["zh_alternatives"]:
      ...
      for test_set in tqdm(sents_f, desc="Loading train dataset"):
          ...
          sents_wer = [0] + [wer(test_set["orig"], s) for s in test_set["zh_alternatives"]]

          max_len_sent = min(max([length(sent) for sent in [test_set["orig"]] + test_set["zh_alternatives"]]), 64)
          for sent in [test_set["orig"]] + test_set["zh_alternatives"]:
              formatted_sent = format_sent(sent)
              if len(sent_inputs) > 0 and formatted_sent == formatted_orig:
                  continue
...
sents_f = json.load(open("../data/alternate_sents_seame_filtered_zh_only.json", 'r'))
...
train_data = load_data(zh[:-2000], evaluate=False)
val_data = load_data(zh[-2000:-1000], evaluate=True)
test_data = load_data(zh[-1000:], evaluate=True)
