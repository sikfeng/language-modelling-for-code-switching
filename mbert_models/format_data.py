...
def load_data(sents_f, evaluate):
  ...
  if evaluate:
      ...
      for test_set in tqdm(sents_f, desc="Loading eval dataset"):
        ...
        for sent in [test_set["orig"]] + test_set["cs_alternatives"] + test_set["zh_alternatives"] + test_set["en_alternatives"]:
      ...
      for test_set in tqdm(sents_f, desc="Loading train dataset"):
          ...
          sents_wer = [0] + [wer(test_set["orig"], s) for s in test_set["cs_alternatives"] + test_set["zh_alternatives"] + test_set["en_alternatives"]]

          max_len_sent = min(max([length(sent) for sent in [test_set["orig"]] + test_set["cs_alternatives"] + test_set["zh_alternatives"] + test_set["en_alternatives"]]), 64)
          for sent in [test_set["orig"]] + test_set["cs_alternatives"] + test_set["zh_alternatives"] + test_set["en_alternatives"]:
              formatted_sent = format_sent(sent)
              if len(sent_inputs) > 0 and formatted_sent == formatted_orig:
                  continue
...
print(len(cs), "cs alt sets")
print(len(en), "en alt sets")
print(len(zh), "zh alt sets")

random.shuffle(cs)
random.shuffle(en)
random.shuffle(zh)

train_data = load_data(en[:-2000] + zh[:-2000] + cs[:-2000], evaluate=False)
val_data = load_data(en[-2000: -1000] + zh[-2000: -1000] + cs[-2000:-1000], evaluate=True)
test_data = load_data(en[-1000:] + zh[-1000:] + cs[-1000:], evaluate=True)
